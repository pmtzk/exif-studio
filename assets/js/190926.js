// Desktop split-colour hero: structural mask, no per-frame geometry reads.
document.addEventListener('DOMContentLoaded', function () {
  ...
  function initSplitColour(){
    ...
    windowMask.className='exif-hero-cream-window';
    windowMask.setAttribute('aria-hidden','true');
    var cream=original.cloneNode(true);
    cream.classList.remove('exif-hero-title-green');
    cream.classList.add('exif-hero-title-cream');
    windowMask.appendChild(cream);
    copy.insertBefore(windowMask,original.nextSibling);
    ...
    function writeMotion(v){var e=1-Math.pow(1-v,3);copy.style.setProperty('--hero-scroll-scale',(1-e*.18).toFixed(5));copy.style.setProperty('--hero-scroll-y',(-(e*42)).toFixed(2)+'px');}
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  function mountGallery(){
    ...
    function makeItems(hidden){return galleryItems.map(function(item,index){var sizeClass='gallery-slot-'+((index%13)+1);return '<figure class="motion-gallery-item '+item[0]+' '+sizeClass+'"'+(hidden?' aria-hidden="true"':'')+'><img src="assets/img/'+item[1]+'" alt="'+(hidden?'':item[2])+'" loading="lazy" decoding="async" fetchpriority="low"></figure>';}).join('')}
    ...
    var galleryScript=document.createElement('script');galleryScript.src='assets/js/motion-gallery.js';galleryScript.defer=true;document.body.appendChild(galleryScript);
  }
  function scheduleGallery(){var run=function(){if('requestIdleCallback'in window)requestIdleCallback(mountGallery,{timeout:1800});else setTimeout(mountGallery,250);};if(document.body.classList.contains('exif-hero-live')&&!document.body.classList.contains('exif-intro-running'))run();else{var hero=document.querySelector('#what-exif-does');if(hero)hero.addEventListener('exif:hero-image-ready',run,{once:true});window.addEventListener('load',function(){setTimeout(run,1800);},{once:true});}}
  scheduleGallery();
