...
document.addEventListener('DOMContentLoaded',function(){
  ...
  function mountGallery(){
    ...
    function makeItems(hidden){return galleryItems.map(function(item,index){var sizeClass='gallery-slot-'+((index%13)+1);var priority=(!hidden&&index<5)?'eager':'lazy';var fetch=(!hidden&&index<5)?' fetchpriority="high"':'';return '<figure class="motion-gallery-item '+item[0]+' '+sizeClass+'"'+(hidden?' aria-hidden="true"':'')+'><img src="assets/img/'+item[1]+'" alt="'+(hidden?'':item[2])+'" loading="'+priority+'" decoding="async"'+fetch+'></figure>';}).join('')}
