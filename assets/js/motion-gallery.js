...
function init(){
  ...
  var pointerId=null,axis=null,startX=0,startY=0,lastX=0,lastPointerT=0,pointerVelocity=0;
  ...
  function frame(now){raf=0;var dt=Math.min(20,Math.max(1,now-lastT));lastT=now,ease=1-Math.exp(-dt/145);revealCurrent+=(revealTarget-revealCurrent)*ease;greenCurrent+=(greenTarget-greenCurrent)*ease;signalCurrent+=(signalTarget-signalCurrent)*ease;
    if(!reduced&&visible()){
      if(mode==='inertia'){position+=velocity*dt;velocity*=Math.exp(-dt/1120);if(Math.abs(velocity)<.044){mode='ambient';velocity=ambientSpeed*desiredDirection}}
  ...
  function onPageScroll(){var y=scrollY,dy=y-lastY;lastY=y;readProgress();if(Math.abs(dy)>1.25&&pointerId===null){desiredDirection=dy>0?1:-1;if(mode==='inertia'&&Math.sign(velocity)!==desiredDirection&&Math.abs(velocity)<.18)mode='ambient'}wake()}
  ...
  viewport.addEventListener('pointerdown',function(e){if(e.pointerType==='mouse'&&e.button!==0)return;pointerId=e.pointerId;axis=null;startX=lastX=e.clientX;startY=e.clientY;lastPointerT=performance.now();pointerVelocity=0;try{viewport.setPointerCapture(pointerId)}catch(_){}},{passive:true});
  viewport.addEventListener('pointermove',function(e){if(e.pointerId!==pointerId)return;var totalX=e.clientX-startX,totalY=e.clientY-startY;if(!axis&&(Math.abs(totalX)>4||Math.abs(totalY)>4)){if(Math.abs(totalX)>=Math.abs(totalY)*.62){axis='x';mode='drag';velocity=0;viewport.classList.add('is-horizontal-drag')}else if(Math.abs(totalY)>Math.abs(totalX)*1.85){axis='y'}}if(axis!=='x')return;if(e.cancelable)e.preventDefault();var now=performance.now(),dt=Math.max(8,now-lastPointerT),dx=e.clientX-lastX;var travel=-dx*.94;position+=travel;var sample=travel/dt;pointerVelocity=pointerVelocity*.78+sample*.22;desiredDirection=sample>=0?1:-1;lastX=e.clientX;lastPointerT=now;renderTrack();wake()},{passive:false});
  function release(e){if(pointerId===null||(e&&e.pointerId!=null&&e.pointerId!==pointerId))return;try{if(viewport.hasPointerCapture(pointerId))viewport.releasePointerCapture(pointerId)}catch(_){}viewport.classList.remove('is-horizontal-drag');pointerId=null;if(axis==='x'){var v=clamp(pointerVelocity*1.16,-1.05,1.05);if(Math.abs(v)>.018){velocity=v;desiredDirection=v>=0?1:-1;mode='inertia'}else{mode='ambient';velocity=ambientSpeed*desiredDirection}}axis=null;wake()}
  viewport.addEventListener('pointerup',release,{passive:true});viewport.addEventListener('pointercancel',release,{passive:true});
  viewport.addEventListener('wheel',function(e){if(Math.abs(e.deltaX)<=Math.abs(e.deltaY)*.62||mode==='drag')return;desiredDirection=e.deltaX>=0?1:-1;velocity=clamp(velocity+e.deltaX*.00038,-.22,.22);mode='inertia';wake()},{passive:true});
