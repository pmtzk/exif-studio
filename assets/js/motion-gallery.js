...
function init(){
  ...
  function frame(now){raf=0;var dt=Math.min(24,Math.max(1,now-lastT));lastT=now,ease=1-Math.exp(-dt/145);revealCurrent+=(revealTarget-revealCurrent)*ease;greenCurrent+=(greenTarget-greenCurrent)*ease;signalCurrent+=(signalTarget-signalCurrent)*ease;
    if(!reduced&&visible()){
      if(mode==='drag'){var follow=1-Math.exp(-dt/82),before=position;position+=(dragTarget-position)*follow;velocity=(position-before)/dt}
      else if(mode==='inertia'){position+=velocity*dt;velocity*=Math.exp(-dt/900);if(Math.abs(velocity)<.048){mode='ambient';velocity=ambientSpeed*desiredDirection}}
      ...
      else{var desired=ambientSpeed*desiredDirection;velocity+=(desired-velocity)*(1-Math.exp(-dt/520));position+=velocity*dt}
      renderTrack()
    }
  ...
  viewport.addEventListener('pointermove',function(e){if(e.pointerId!==pointerId)return;var totalX=e.clientX-startX,totalY=e.clientY-startY;if(!axis&&(Math.abs(totalX)>4||Math.abs(totalY)>4)){if(Math.abs(totalX)>=Math.abs(totalY)*.62){axis='x';mode='drag';dragTarget=position;velocity=0;viewport.classList.add('is-horizontal-drag')}else if(Math.abs(totalY)>Math.abs(totalX)*1.85){axis='y'}}if(axis!=='x')return;if(e.cancelable)e.preventDefault();var now=performance.now(),dt=Math.max(8,now-lastPointerT),dx=e.clientX-lastX;dragTarget-=dx;var sample=-dx/dt;pointerVelocity=pointerVelocity*.84+sample*.16;desiredDirection=sample>=0?1:-1;lastX=e.clientX;lastPointerT=now;wake()},{passive:false});
  function release(e){if(pointerId===null||(e&&e.pointerId!=null&&e.pointerId!==pointerId))return;try{if(viewport.hasPointerCapture(pointerId))viewport.releasePointerCapture(pointerId)}catch(_){}viewport.classList.remove('is-horizontal-drag');pointerId=null;if(axis==='x'){var residual=dragTarget-position,v=clamp(pointerVelocity*.58+residual*.0022,-.82,.82);position+=residual*.12;if(Math.abs(v)>.022){velocity=v;desiredDirection=v>=0?1:-1;mode='inertia'}else{mode='ambient';velocity=ambientSpeed*desiredDirection}}axis=null;wake()}
