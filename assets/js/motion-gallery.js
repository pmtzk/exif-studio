  ...
  function frame(now){raf=0;var dt=Math.min(20,Math.max(1,now-lastT));lastT=now,ease=1-Math.exp(-dt/145);revealCurrent+=(revealTarget-revealCurrent)*ease;greenCurrent+=(greenTarget-greenCurrent)*ease;signalCurrent+=(signalTarget-signalCurrent)*ease;
    if(!reduced&&visible()){
      ...
      if(mode==='drag'||mode==='settle'){
        var follow=1-Math.exp(-dt/20),delta=(targetX-position)*follow,limit=maxVisualStep*(dt/16.667);position+=clamp(delta,-limit,limit);
        if(mode==='settle'&&Math.abs(targetX-position)<.12){position=targetX;bounceTarget=clamp(desiredDirection*1.35,-1.35,1.35);mode='ambient';velocity=ambientSpeed*desiredDirection}
      }
