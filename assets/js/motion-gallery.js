...
function init(){
  ...
  function onPageScroll(){var y=scrollY,dy=y-lastY;lastY=y;readProgress();if(Math.abs(dy)>1.25&&pointerId===null&&mode!=='wheel'){desiredDirection=dy>0?1:-1;targetX+=clamp(dy*.18,-8,8);mode='settle'}wake()}
  ...
  viewport.addEventListener('wheel',function(e){if(mode==='drag')return;var horizontal=Math.abs(e.deltaX)>Math.abs(e.deltaY)*.34,delta=horizontal?e.deltaX:e.deltaY;if(Math.abs(delta)<.2)return;if(e.cancelable)e.preventDefault();desiredDirection=delta>=0?1:-1;if(mode!=='wheel'){targetX=position;bounceOffset=desiredDirection*.65;bounceTarget=0}targetX+=clamp(delta*.42,-14,14);mode='wheel';settleSoon();wake()},{passive:false});
