...
function init(){
  ...
  function frame(now){raf=0;var rawDt=Math.max(1,now-lastT),dt=Math.min(34,rawDt);lastT=now;if(progressDirty)readProgress();var progressEase=1-Math.exp(-dt/145);revealCurrent+=(revealTarget-revealCurrent)*progressEase;greenCurrent+=(greenTarget-greenCurrent)*progressEase;signalCurrent+=(signalTarget-signalCurrent)*progressEase;
    if(!reduced&&isVisible){
      ...
      if(mode==='drag'||mode==='wheel'){
        ...
        velocity+=(desiredAmbient-velocity)*(1-Math.exp(-dt/95));
        position+=velocity*dt*near;
        if(Math.abs(targetX-position)<.12){targetX=position;mode='ambient';bounceTarget=desiredDirection*.45;velocity=desiredAmbient}
      }else if(mode==='button'){
    ...
    paintProgress();
    var progressMoving=Math.abs(revealTarget-revealCurrent)>.001||Math.abs(greenTarget-greenCurrent)>.001||Math.abs(signalTarget-signalCurrent)>.001;
    if((isVisible&&!reduced)||progressMoving)raf=requestAnimationFrame(frame)
  }
  ...
  measureLoop();targetX=position;renderTrack();readProgress();revealCurrent=revealTarget;greenCurrent=greenTarget;signalCurrent=signalTarget;paintProgress();
  window.addEventListener('scroll',onPageScroll,{passive:true});window.addEventListener('resize',function(){measureLoop();progressDirty=true;renderTrack();wake()},{passive:true});
  if('ResizeObserver'in window)new ResizeObserver(function(){var old=loopWidth;measureLoop();if(Math.abs(loopWidth-old)>.5){targetX=position;renderTrack()}}).observe(track);
  if('IntersectionObserver'in window)new IntersectionObserver(function(entries){isVisible=entries[0].isIntersecting;if(isVisible){measureLoop();progressDirty=true;renderTrack();wake()}},{threshold:.01}).observe(chapter);
  wake();
}
