...
function init(){
  ...
  var dragRatio=.90,maxDragSpeed=.34,maxReleaseSpeed=.16;
  ...
  function frame(now){raf=0;var dt=Math.min(20,Math.max(1,now-lastT));lastT=now,ease=1-Math.exp(-dt/145);revealCurrent+=(revealTarget-revealCurrent)*ease;greenCurrent+=(greenTarget-greenCurrent)*ease;signalCurrent+=(signalTarget-signalCurrent)*ease;
    if(!reduced&&visible()){
      if(mode==='settle'){var step=Math.sign(releaseLeft)*Math.min(Math.abs(releaseLeft),Math.abs(velocity)*dt);position+=step;releaseLeft-=step;velocity*=Math.exp(-dt/105);if(Math.abs(releaseLeft)<.35||Math.abs(velocity)<.012){mode='ambient';releaseLeft=0;velocity=ambientSpeed*desiredDirection}}
  ...
  viewport.addEventListener('pointermove',function(e){if(e.pointerId!==pointerId)return;var totalX=e.clientX-startX,totalY=e.clientY-startY;if(!axis&&(Math.abs(totalX)>3||Math.abs(totalY)>3)){if(Math.abs(totalX)>=Math.abs(totalY)*.42){axis='x';mode='drag';velocity=0;viewport.classList.add('is-horizontal-drag')}else if(Math.abs(totalY)>Math.abs(totalX)*2.6){axis='y'}}if(axis!=='x')return;if(e.cancelable)e.preventDefault();var now=performance.now(),dt=Math.max(8,now-lastPointerT),dx=e.clientX-lastX;var raw=-dx*dragRatio,limit=maxDragSpeed*dt,travel=clamp(raw,-limit,limit);position+=travel;var sample=travel/dt;pointerVelocity=pointerVelocity*.70+sample*.30;desiredDirection=sample>=0?1:-1;lastX=e.clientX;lastPointerT=now;renderTrack();wake()},{passive:false});
  function release(e){if(pointerId===null||(e&&e.pointerId!=null&&e.pointerId!==pointerId))return;try{if(viewport.hasPointerCapture(pointerId))viewport.releasePointerCapture(pointerId)}catch(_){}viewport.classList.remove('is-horizontal-drag');pointerId=null;if(axis==='x'){var v=clamp(pointerVelocity,-maxReleaseSpeed,maxReleaseSpeed);if(Math.abs(v)>.012){velocity=v;desiredDirection=v>=0?1:-1;releaseLeft=clamp(v*115,-18,18);mode='settle'}else{mode='ambient';velocity=ambientSpeed*desiredDirection}}axis=null;wake()}
  viewport.addEventListener('pointerup',release,{passive:true});viewport.addEventListener('pointercancel',release,{passive:true});
  viewport.addEventListener('wheel',function(e){if(Math.abs(e.deltaX)<=Math.abs(e.deltaY)*.48||mode==='drag')return;desiredDirection=e.deltaX>=0?1:-1;velocity=clamp(velocity+e.deltaX*.00025,-.16,.16);releaseLeft=clamp(velocity*100,-16,16);mode='settle';wake()},{passive:true});
