...
function init(){
  ...
  var dragRatio=.97,maxDragSpeed=.58,maxReleaseSpeed=.12;
  ...
  function frame(now){raf=0;var dt=Math.min(20,Math.max(1,now-lastT));lastT=now,ease=1-Math.exp(-dt/145);revealCurrent+=(revealTarget-revealCurrent)*ease;greenCurrent+=(greenTarget-greenCurrent)*ease;signalCurrent+=(signalTarget-signalCurrent)*ease;
    if(!reduced&&visible()){
      if(mode==='settle'){var step=Math.sign(releaseLeft)*Math.min(Math.abs(releaseLeft),Math.max(.18,Math.abs(velocity)*dt));position+=step;releaseLeft-=step;velocity*=Math.exp(-dt/72);if(Math.abs(releaseLeft)<.25||Math.abs(velocity)<.009){mode='ambient';releaseLeft=0;velocity=ambientSpeed*desiredDirection}}
  ...
  viewport.addEventListener('pointermove',function(e){if(e.pointerId!==pointerId)return;var totalX=e.clientX-startX,totalY=e.clientY-startY,ax=Math.abs(totalX),ay=Math.abs(totalY);if(!axis&&(ax>2||ay>2)){if(ax>=2&&ax>=ay*.28){axis='x';mode='drag';velocity=0;viewport.classList.add('is-horizontal-drag')}else if(ay>10&&ay>ax*3.5){axis='y'}}if(axis!=='x')return;if(e.cancelable)e.preventDefault();var now=performance.now(),dt=Math.max(8,now-lastPointerT),dx=e.clientX-lastX;var raw=-dx*dragRatio,limit=maxDragSpeed*dt,travel=clamp(raw,-limit,limit);position+=travel;var sample=travel/dt;pointerVelocity=pointerVelocity*.62+sample*.38;if(Math.abs(sample)>.002)desiredDirection=sample>=0?1:-1;lastX=e.clientX;lastPointerT=now;renderTrack();wake()},{passive:false});
  function release(e){if(pointerId===null||(e&&e.pointerId!=null&&e.pointerId!==pointerId))return;try{if(viewport.hasPointerCapture(pointerId))viewport.releasePointerCapture(pointerId)}catch(_){}viewport.classList.remove('is-horizontal-drag');pointerId=null;if(axis==='x'){var v=clamp(pointerVelocity,-maxReleaseSpeed,maxReleaseSpeed);if(Math.abs(v)>.01){velocity=v;desiredDirection=v>=0?1:-1;releaseLeft=clamp(v*58,-7,7);mode='settle'}else{mode='ambient';velocity=ambientSpeed*desiredDirection}}axis=null;wake()}
  viewport.addEventListener('pointerup',release,{passive:true});viewport.addEventListener('pointercancel',release,{passive:true});
  viewport.addEventListener('wheel',function(e){if(Math.abs(e.deltaX)<=Math.abs(e.deltaY)*.34||mode==='drag')return;desiredDirection=e.deltaX>=0?1:-1;velocity=clamp(velocity+e.deltaX*.00022,-.12,.12);releaseLeft=clamp(velocity*52,-7,7);mode='settle';wake()},{passive:true});
