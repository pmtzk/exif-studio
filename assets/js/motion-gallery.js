...
function init(){
  ...
  function frame(now){raf=0;var dt=Math.min(20,Math.max(1,now-lastT));lastT=now,ease=1-Math.exp(-dt/145);revealCurrent+=(revealTarget-revealCurrent)*ease;greenCurrent+=(greenTarget-greenCurrent)*ease;signalCurrent+=(signalTarget-signalCurrent)*ease;
    if(!reduced&&visible()){
      if(mode==='drag'){
        var error=dragTarget-position;
        var spring=error*.00034;
        velocity+=spring*dt;
        velocity*=Math.exp(-dt/92);
        position+=velocity*dt;
      }
      else if(mode==='inertia'){position+=velocity*dt;velocity*=Math.exp(-dt/980);if(Math.abs(velocity)<.046){mode='ambient';velocity=ambientSpeed*desiredDirection}}
  ...
  viewport.addEventListener('pointermove',function(e){if(e.pointerId!==pointerId)return;var totalX=e.clientX-startX,totalY=e.clientY-startY;if(!axis&&(Math.abs(totalX)>4||Math.abs(totalY)>4)){if(Math.abs(totalX)>=Math.abs(totalY)*.62){axis='x';mode='drag';dragTarget=position;velocity*=.35;viewport.classList.add('is-horizontal-drag')}else if(Math.abs(totalY)>Math.abs(totalX)*1.85){axis='y'}}if(axis!=='x')return;if(e.cancelable)e.preventDefault();var now=performance.now(),dt=Math.max(8,now-lastPointerT),dx=e.clientX-lastX;dragTarget-=dx;var sample=-dx/dt;pointerVelocity=pointerVelocity*.72+sample*.28;desiredDirection=sample>=0?1:-1;lastX=e.clientX;lastPointerT=now;wake()},{passive:false});
  function release(e){if(pointerId===null||(e&&e.pointerId!=null&&e.pointerId!==pointerId))return;try{if(viewport.hasPointerCapture(pointerId))viewport.releasePointerCapture(pointerId)}catch(_){}viewport.classList.remove('is-horizontal-drag');pointerId=null;if(axis==='x'){var residual=dragTarget-position;velocity=clamp(velocity+pointerVelocity*.18+residual*.0018,-.78,.78);if(Math.abs(velocity)>.02||Math.abs(residual)>1){desiredDirection=velocity>=0?1:-1;mode='inertia'}else{mode='ambient';velocity=ambientSpeed*desiredDirection}}axis=null;wake()}
