...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* HERO ONLY — cream -> image sequence -> final frame expands -> quiet UI reveal. */
  ...
  style.textContent = `
    ...
    /* Overlay the existing header on the live hero without changing the drawer's own layout. */
    ...
    .exif-loader-stage{position:relative;width:12px;height:12px;background:#1c362a;overflow:hidden;will-change:width,height;transition:width 1.15s cubic-bezier(.76,0,.24,1),height 1.15s cubic-bezier(.76,0,.24,1)}
    ...
    .exif-hero-copy h1{margin:0;color:#f0e8dd;font-family:var(--font-serif);font-size:clamp(4rem,10vw,10rem);font-weight:400;line-height:.79;letter-spacing:-.055em;text-transform:uppercase;max-width:10ch}
    ...
    .exif-hero-meta{display:flex;justify-content:space-between;align-items:end;margin-top:1.35rem;font:500 9px/1.25 'Jost',sans-serif;letter-spacing:.14em;text-transform:uppercase;opacity:0;transform:translateY(8px);transition:opacity 1s ease .85s,transform 1s ease .85s}
    ...
    @media(prefers-reduced-motion:reduce){.exif-loader{display:none}.exif-hero-copy .word{opacity:1;transform:none;filter:none}.exif-hero-meta{opacity:.82;transform:none}}
  `;
  ...
  setTimeout(function(){
    ...
    function flash(){
      ...
      if(i<imgs.length){
        ...
        /* Give the final image a beat, then visibly expand that exact frame to full bleed. */
        setTimeout(function(){
          stage.classList.add('is-full');
          /* UI waits until the expansion is completely finished. */
          setTimeout(function(){
            document.body.classList.add('exif-hero-live');
