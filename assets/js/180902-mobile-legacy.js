...
document.addEventListener('DOMContentLoaded', function () {
  ...
  style.textContent = `
    @media(max-width:859px){
      ...
      body.exif-mobile-legacy.exif-hero-live:not(.nav-open) .site-header .brand-logo{filter:brightness(0) invert(1)!important;opacity:.96!important;width:72px!important;height:auto!important}
  ...
  /* Remove any hero generated by the desktop/current controller before rebuilding the approved mobile state. */
  ...
  if(reduced){document.body.classList.add('exif-hero-live');hero.querySelector('.exif-hero-copy').classList.add('is-visible');window.addEventListener('scroll',setHeader,{passive:true});return;}

  /* Kill a loader started by the current desktop controller on mobile and own the sequence here. */
