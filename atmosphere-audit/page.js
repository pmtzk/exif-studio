...
const compareObserver=new IntersectionObserver(entries=>entries.forEach(e=>{if(e.isIntersecting&&!comparePlayed){comparePlayed=true;renderCompare('person',true);if(!reduced){compareTimers.push(setTimeout(()=>renderCompare('online',true),2350));compareTimers.push(setTimeout(()=>$('[data-comparison-question]').classList.add('is-visible'),4550))}else $('[data-comparison-question]').classList.add('is-visible');compareObserver.disconnect()}}),{threshold:.28});compareObserver.observe($('[data-comparison]'));
